...
func GetLogByKey(key string) (logs []*Log, err error) {
	...
	// Use created_at for ordering to support ClickHouse where ID might be 0
...
func SearchAllLogs(keyword string) (logs []*Log, err error) {
	// Use created_at for ordering to support ClickHouse where ID might be 0
	if common.LogSqlType == common.DatabaseTypeClickHouse {
		err = LOG_DB.Where("type = ? or content LIKE ?", keyword, keyword+"%").Order("created_at desc, id desc").Limit(common.MaxRecentItems).Find(&logs).Error
	} else {
...
func SearchUserLogs(userId int, keyword string) (logs []*Log, err error) {
	// Use created_at for ordering to support ClickHouse where ID might be 0
	if common.LogSqlType == common.DatabaseTypeClickHouse {
		err = LOG_DB.Where("user_id = ? and type = ?", userId, keyword).Order("created_at desc, id desc").Limit(common.MaxRecentItems).Find(&logs).Error
	} else {
